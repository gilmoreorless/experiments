/*
 * Original file from http://libccv.org
...
var helpers = {
    region: function (name) {
        ...
        region.id = name;
        region.innerHTML = '<h1>' + name + '</h1>';
        document.body.appendChild(region);
        currentRegion = region;
        return region;
    },
    regionSection: function (name) {
        var tag = document.createElement('h3');
        tag.innerHTML = name;
        currentRegion.appendChild(tag);
        return tag;
    },
    block: function (name, child) {
        ...
        currentRegion.appendChild(block);
        return block;
    }
}
// END Gil debug
...
var ccv = {
    ...
    detect_objects : parallable("ccv.js", function (canvas, cascade, interval, min_neighbors) {
        ...
        function pre(worker_num) {
            ...
            var i;
            helpers.regionSection('1 -> interval');
            for (i = 1; i <= interval; i++) {
                ...
                helpers.block(i * 4, pyr[i * 4]);
            }
            helpers.regionSection('next -> scale_upto + next * 2');
            ...
            helpers.regionSection('next * 2 -> scale_upto + next * 2');
        ...
        function core(pyr, id, worker_num) {
            ...
            for (i = 0; i < scale_upto; i++) {
                ...
                console.log('qw: %d, qh: %d, step:', qw, qh, step, ', paddings:', paddings);
                ...
                for (q = 0; q < 4; q++) {
                    ...
                    console.log('q: %d, u8:', q, u8, 'u8o:', u8o.slice());
                    for (y = 0; y < qh; y++) {
                        for (x = 0; x < qw; x++) {
                            var sum = 0;
                            var flag = true;
                            for (j = 0; j < cascade.stage_classifier.length; j++) {
                                ...
                                for (k = 0; k < cascade.stage_classifier[j].count; k++) {
                                    ...
                                    // console.log('y: %d, x: %d, j: %d, feature_k: %o, pmin: %f, nmax: %f', y, x, j, feature_k, pmin, nmax);
                                    if (pmin <= nmax) {
                                        sum += alpha[k * 2];
                                    } else {
                                        var f, shortcut = true;
        ...
        function post(seq) {
            ...
            if (!(min_neighbors > 0))
                ...
                var tmpCanvas = document.createElement('canvas');
                var srcCanvas = this.shared.canvas;
                tmpCanvas.width = srcCanvas.width * 2;
                tmpCanvas.height = srcCanvas.height * 2;
                var tmpCtx = tmpCanvas.getContext('2d');
                tmpCtx.drawImage(
                    srcCanvas,
                    0, 0, srcCanvas.width, srcCanvas.height,
                    0, 0, srcCanvas.width * 2, srcCanvas.height * 2
                );
                helpers.block('before grouping', tmpCanvas);
                tmpCtx.strokeStyle = '#0F0';
                tmpCtx.font = '10px Arial';
                i = seq.length;
                while (i--) {
                    face = seq[i];
                    tmpCtx.strokeRect(face.x * 2, face.y * 2, face.width * 2, face.height * 2);
                    tmpCtx.strokeText(face.confidence.toFixed(4), face.x * 2, face.y * 2 - 5);
                }
                ...
                helpers.block('after grouping and averaging', tmpCanvas);
                tmpCtx.strokeStyle = '#0F0';
                tmpCtx.font = '10px Arial';
                i = seq2.length;
                while (i--) {
